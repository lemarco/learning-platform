import { allNamesInFolder, filterFolders, filterTsFiles, isClient, isDot, isLayout, isServer } from "./utils";
import { $ } from "bun";
type RouteObj = {
  "client.ts"?: string;
  "index.ts"?: string;
...
export const builder = async (input: string, outPath: string) => {
  ...
  if (isClient(input)) {
    await Bun.write(
      ...
      (await Bun.file(output.outputs[0].path).text()).replace(/export{([a-zA-Z]) as client};/g, (_, letter) => `${letter}()`),
    );
  } else {
    await Bun.write(
      output.outputs[0].path,
      (await Bun.file(output.outputs[0].path).text()).replace(/[\t\n]/g, "").replaceAll('className=""', ""),
    );
  }
...
export const startBuild = async (path = "./pages", outpath = "./dist") => {
