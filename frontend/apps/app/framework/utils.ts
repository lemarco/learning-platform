...
const FileTypes = ["layout.ts", "client.ts", "page.ts", "loader.ts", "error.ts"] as const;
...
export const isDynamicFolder = (file: string) => isFolder(file) && FolderTypes.some((prefix) => file.startsWith(prefix));
export const isFile = (file: string) => file.endsWith(".ts");
export const isFileToMatch = (file: string) => isFile(file) && FileTypes.some((type) => file === type);
export const removeBase = (url: string, base: string) => url.replace(base, "");
export const splitUrl = (url: string) => url.split("/");
export const removeBaseAndSplit = (url: string, base: string) => splitUrl(removeBase(url, base)).filter(Boolean);
export const getAllFilesPaths = (dir = "./pages") => readdirSync(dir, { recursive: true });
export const isPage = (path: string | Buffer) => (path as string).endsWith("page.ts");
export const getAllPagesPaths = (dir = "./pages") => getAllFilesPaths(dir).filter(isPage).filter(Boolean) as string[];

export const ls = async (path: string) => await $`ls ./${path}`.text();
export const allNamesInFolder = async (path: string) => (await ls(path)).split("\n");

export const isTsOrTsx = (file: string) => file.endsWith(".tsx") || file.endsWith(".ts");

export const filterTsFiles = (list: string[]) => list.filter(isTsOrTsx);
export const filterFolders = (list: string[]) => list.filter((file) => !isTsOrTsx(file));
