...
import { $findMatchingParent, $insertNodeToNearestRoot, mergeRegister } from "../../../";
import {
  $createParagraphNode,
  ...
  useEffect(() => {
    ...
    const onEscapeUp = () => {
      ...
      if ($isRangeSelection(selection) && selection.isCollapsed() && selection.anchor.offset === 0) {
        ...
        if ($isCollapsibleContainerNode(container)) {
          ...
          if (
            parent !== null &&
            parent.getFirstChild<EditorNode>() === container &&
            selection.anchor.key === container.getFirstDescendant<EditorNode>()?.getKey()
          ) {
    ...
    const onEscapeDown = () => {
      ...
      if ($isRangeSelection(selection) && selection.isCollapsed()) {
        ...
        if ($isCollapsibleContainerNode(container)) {
          const parent = container.getParent<ElementNode>();
          if (parent !== null && parent.getLastChild<EditorNode>() === container) {
            const lastDescendant = container.getLastDescendant<EditorNode>();
            if (
              lastDescendant !== null &&
    ...
    return mergeRegister(
      ...
      editor.registerNodeTransform(CollapsibleContentNode, (node) => {
        ...
        if (!$isCollapsibleContainerNode(parent)) {
          const children = node.getChildren<EditorNode>();
      ...
      editor.registerNodeTransform(CollapsibleTitleNode, (node) => {
        const parent = node.getParent<ElementNode>();
        if (!$isCollapsibleContainerNode(parent)) {
          node.replace($createParagraphNode().append(...node.getChildren<EditorNode>()));
          return;
        }
      }),
      ...
      editor.registerCommand(
        ...
        () => {
          ...
          // @ts-ignore
          const container = topLevelElement.getPreviousSibling<EditorNode>();
