...
import type { DEPRECATED_GridCellNode, ElementNode, LexicalEditor } from "../../../../editor";

import { useLexicalComposerContext } from "../../../../editor";
import { useLexicalEditable } from "../../../../editor";
import {
  ...
  useEffect(() => {
    editor.getEditorState().read(() => {
      ...
      if (DEPRECATED_$isGridSelection(selection)) {
        ...
        setCanMergeCells(isGridSelectionRectangular(selection) && (currentSelectionCounts.columns > 1 || currentSelectionCounts.rows > 1));
      }
      // Unmerge cell
  ...
  const clearTableSelection = useCallback(() => {
    editor.update(() => {
      if (tableCellNode.isAttached()) {
        const tableNode = $getTableNodeFromEditorNodeOrThrow(tableCellNode);
  ...
  return createPortal(
    ...
    >
      ...
      <button
        className="item"
        onClick={() =>
          showColorPickerModal("Cell background color", () => <ColorPicker color={backgroundColor} onChange={handleCellBackgroundColor} />)
        }
...
function TableCellActionMenuContainer({
  ...
  const moveMenu = useCallback(() => {
    ...
    if (
      ...
      const tableCellNodeFromSelection = $getTableCellNodeFromEditorNode(selection.anchor.getNode());
