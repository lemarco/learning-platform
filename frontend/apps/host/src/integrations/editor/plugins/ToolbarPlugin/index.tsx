...
interface ToolbarPluginProps extends DetailedHTMLProps<HtmlHTMLAttributes<HTMLDivElement>, HTMLDivElement> {
  sendState: () => void;
}

export function ToolbarPlugin({ className, sendState }: ToolbarPluginProps): JSX.Element {
  ...
  return (
    <div className={className}>
      ...
      <ModeButton editor={editor} blockType={blockType as unknown as keyof typeof blockTypeToBlockName} />
      <Divider />
      {blockType === "code" ? (
        <CodeView
          editor={editor}
      ...
      <Divider />
      <AlignDropDown editor={editor} setActiveEditor={setActiveEditor} isEditable={isEditable} activeEditor={activeEditor} />
      {modal}
    </div>
  );
}
