...
import { $isLinkNode, LinkNode } from "../link";
import { $isMarkNode } from "../mark";
...
function generateContent(
  ...
  const selectionString = editorState.read(() => {
    ...
    visitTree($getRoot(), (node: EditorNode, indent: Array<string>) => {
...
function visitTree(
  ...
  visitor: (node: EditorNode, indentArr: Array<string>) => void,
  indent: Array<string> = []
) {
...
function printNode(node: EditorNode) {
  if ($isTextNode(node)) {
...
const FORMAT_PREDICATES = [
  (node: EditorNode | RangeSelection) => node.hasFormat("bold") && "Bold",
  (node: EditorNode | RangeSelection) => node.hasFormat("code") && "Code",
  (node: EditorNode | RangeSelection) => node.hasFormat("italic") && "Italic",
  (node: EditorNode | RangeSelection) =>
...
const DETAIL_PREDICATES = [
  (node: EditorNode) => node.isDirectionless() && "Directionless",
  (node: EditorNode) => node.isUnmergeable() && "Unmergeable",
];

const MODE_PREDICATES = [
  (node: EditorNode) => node.isToken() && "Token",
  (node: EditorNode) => node.isSegmented() && "Segmented",
];

function printAllTextNodeProperties(node: EditorNode) {
  return [
...
function printDetailProperties(nodeOrSelection: EditorNode) {
...
function printModeProperties(nodeOrSelection: EditorNode) {
...
function printFormatProperties(nodeOrSelection: EditorNode | RangeSelection) {
