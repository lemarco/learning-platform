 ...
 */
export function $cloneWithProperties<T extends EditorNode>(node: T): T {
...
export function trimTextContentFromAnchor(
  ...
  let currentNode: EditorNode | null = anchor.getNode();
  ...
  while (remaining > 0 && currentNode !== null) {
    let nextNode: EditorNode | null = currentNode.getPreviousSibling();
    let additionalElementWhitespace = 0;
    if (nextNode === null) {
      let parent: EditorNode | null = currentNode.getParentOrThrow();
      let parentSibling: EditorNode | null = parent.getPreviousSibling();
