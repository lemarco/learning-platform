...
export function $updateDOMForSelection(
  ...
  $forEachGridCell(grid, (cell, EditorNode) => {
    const elem = cell.elem;

    if (selectedCellNodes.has(EditorNode)) {
...
export function $forEachGridCell(
  ...
  for (let y = 0; y < cells.length; y++) {
    ...
    for (let x = 0; x < row.length; x++) {
      ...
      const EditorNode = $getNearestNodeFromDOMNode(cell.elem);
...
function $addHighlightToDOM(editor: LexicalEditor, cell: Cell): void {
  ...
  invariant(
    ...
    "Expected to find EditorNode from Table Cell DOMNode"
  );
...
function $findCellNode(node: EditorNode): null | TableCellNode {
  const cellNode = $findMatchingParent(node, $isTableCellNode);
  return $isTableCellNode(cellNode) ? cellNode : null;
}

function $findTableNode(node: EditorNode): null | TableNode {
