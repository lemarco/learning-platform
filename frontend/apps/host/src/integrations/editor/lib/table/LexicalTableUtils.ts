...
import type { DEPRECATED_GridRowNode, ElementNode } from "../";
...
export function $getTableCellNodeFromEditorNode(
...
export function $getTableNodeFromEditorNodeOrThrow(
  startingNode: EditorNode
): TableNode {
...
export function $getTableRowIndexFromTableCellNode(
  ...
  const tableNode = $getTableNodeFromEditorNodeOrThrow(tableRowNode);
...
export function $deleteTableColumn__EXPERIMENTAL(): void {
  ...
  const { startRow: focusStartRow, startColumn: focusStartColumn } =
    focusCellMap;
...
function $insertFirst(parent: ElementNode, node: EditorNode): void {
