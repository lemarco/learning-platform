import { $generateHtmlFromNodes, $generateNodesFromDOM } from "../html";
import {
  $addNodeStyle,
  $cloneWithProperties,
  $sliceSelectedTextNodeContent,
} from "../selection";
import { $findMatchingParent, objectKlassEquals } from "../utils";
import {
  $createParagraphNode,
  $createTabNode,
  $getRoot,
...
import { invariant, CAN_USE_DOM } from "shared/utils";
 ...
 * logic defined in the exportDOM methods on the EditorNode classes. Note that
 ...
 * logic defined in the exportJSON methods on the EditorNode classes. Note that
...
function exportNodeToJSON<T extends EditorNode>(node: T): BaseSerializedNode {
  ...
  if (serializedNode.type !== nodeClass.getType()) {
    invariant(
      false,
      "EditorNode: Node %s does not implement .exportJSON().",
      nodeClass.name
    );
  }
 ...
 * an Array containing instances of the corresponding EditorNode classes registered on the editor.
