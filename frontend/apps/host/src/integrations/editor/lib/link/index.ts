...
import { addClassNamesToElement, isHTMLAnchorElement } from "../utils";
import {
...
function $getLinkAncestor(node: EditorNode): null | EditorNode {
  return $getAncestor(node, $isLinkNode);
}

function $getAncestor<NodeType extends EditorNode = EditorNode>(
  node: EditorNode,
  predicate: (ancestor: EditorNode) => ancestor is NodeType
): null | EditorNode {
  let parent: null | EditorNode = node;
  while (
    parent !== null &&
