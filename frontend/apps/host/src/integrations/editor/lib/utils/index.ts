...
import { $cloneWithProperties } from "../selection";
import {
  $createParagraphNode,
 ...
 * {depth: number, node: EditorNode} It will always return at least 1 node (the ending node) so long as it exists
 */
export function $dfs(
  startingNode?: EditorNode,
  endingNode?: EditorNode
): Array<DFSNode> {
  const nodes = [];
  const start = (startingNode || $getRoot()).getLatest();
  const end =
    endingNode || ($isElementNode(start) ? start.getLastDescendant() : start);
  let node: EditorNode | null = start;
...
function $getDepth(node: EditorNode): number {
  let innerNode: EditorNode | null = node;
  let depth = 0;
...
export function $getNearestNodeOfType<T extends ElementNode>(
  ...
  let parent: ElementNode | EditorNode | null = node;
...
export type DOMNodeToLexicalConversion = (element: Node) => EditorNode;
...
export function $findMatchingParent(
  startingNode: EditorNode,
  findFn: (node: EditorNode) => boolean
): EditorNode | null {
  let curr: ElementNode | EditorNode | null = startingNode;
...
export function registerNestedElementResolver<N extends ElementNode>(
  ...
  const $isTargetNode = (node: EditorNode | null | undefined): node is N => {
 ...
 */
export function $insertNodeToNearestRoot<T extends EditorNode>(node: T): T {
