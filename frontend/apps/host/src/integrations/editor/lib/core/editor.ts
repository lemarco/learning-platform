import type { EditorState, SerializedEditorState } from "./editorState";
import type { DOMConversion, NodeKey } from "./nodes/editor";

import { invariant } from "shared/utils";

import { $getRoot, $getSelection, TextNode } from ".";
import { FULL_RECONCILE, NO_DIRTY_NODES } from "./constants";
...
import { DecoratorNode } from "./nodes/decorator";
...
export type Klass<T extends EditorNode> = {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  new (...args: any[]): T;
} & Omit<EditorNode, "constructor">;
...
export type RegisteredNode = {
  ...
  transforms: Set<Transform<EditorNode>>;
  replace: null | ((node: EditorNode) => EditorNode);
  replaceWithKlass: null | Klass<EditorNode>;
};

export type Transform<T extends EditorNode> = (node: T) => void;

export type ErrorHandler = (error: Error) => void;

export type MutationListeners = Map<MutationListener, Klass<EditorNode>>;

export type MutatedNodes = Map<Klass<EditorNode>, Map<NodeKey, NodeMutation>>;
...
export class LexicalEditor {
  ...
  private registerNodeTransformToKlass<T extends EditorNode>(
    klass: Klass<T>,
    ...
    transforms?.add(listener as Transform<EditorNode>);
   ...
   * @returns a teardown function that can be used to cleanup the listener.
   */
  registerNodeTransform<T extends EditorNode>(
    klass: Klass<T>,
    listener: Transform<T>
  ): () => void {
    ...
    if (replaceWithKlass != null) {
      const registeredReplaceWithNode = this.registerNodeTransformToKlass(
        replaceWithKlass,
        listener as Transform<EditorNode>
      );
    ...
    return () => {
      registeredNodes.forEach(
        (node) => node?.transforms.delete(listener as Transform<EditorNode>)
      );
    };
  }
   ...
   */
  hasNode<T extends Klass<EditorNode>>(node: T): boolean {
   ...
   */
  hasNodes<T extends Klass<EditorNode>>(nodes: Array<T>): boolean {
  ...
  /**
   * Gets the underlying HTMLElement associated with the EditorNode for the given key.
   * @returns the HTMLElement rendered by the EditorNode associated with the key.
   * @param key - the key of the EditorNode.
   */
   ...
   *
   * See {@link EditorNode.exportJSON}
   *
