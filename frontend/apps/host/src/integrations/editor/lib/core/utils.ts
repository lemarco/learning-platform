...
import type { EditorNode, NodeKey, NodeMap } from "./nodes/editor";
import type {
  GridSelection,
  NodeSelection,
  PointType,
...
import type { TextFormatType, TextNode } from "./nodes/text";

import { CAN_USE_DOM } from "shared/utils";
import { IS_APPLE, IS_APPLE_WEBKIT, IS_IOS, IS_SAFARI } from "shared/utils";
...
import { $normalizeSelection } from "./normalization";
import {
...
export function removeFromParent(node: EditorNode): void {
...
export function internalMarkNodeAsDirty(node: EditorNode): void {
  errorOnInfiniteTransforms();
...
export function internalMarkSiblingsAsDirty(node: EditorNode) {
...
export function $getNodeByKey<T extends EditorNode>(
  key: NodeKey,
...
export function getNodeFromDOM(dom: Node): null | EditorNode {
  const editor = getActiveEditor();
...
export function $selectAllUtil(): void {
  const root = $getRoot();
...
export function $nodesOfType<T extends EditorNode>(klass: Klass<T>): Array<T> {
  const editorState = getActiveEditorState();
...
export function $isInlineElementOrDecoratorNode(node: EditorNode): boolean {
  return (
...
export function $isRootOrShadowRoot(node: null | EditorNode): boolean {
  return $isRootNode(node) || ($isElementNode(node) && node.isShadowRoot());
}

export function $copyNode<T extends EditorNode>(node: T): T {
  // @ts-ignore
...
export function $applyNodeReplacement<N extends EditorNode>(
  node: EditorNode
): N {
  const editor = getActiveEditor();
  const nodeType = (node.constructor as Klass<EditorNode>).getType();
  const registeredNode = editor._nodes.get(nodeType);
  if (registeredNode === undefined) {
    invariant(
      false,
...
// export function $getNodeByKeyOrThrow<N extends EditorNode>(key: NodeKey): N {
...
function needsBlockCursor(node: null | EditorNode): boolean {
  return (
...
export function updateDOMBlockCursorElement(
  ...
  if (
    ...
    if (offset === elementNodeSize) {
      ...
      if (needsBlockCursor(child)) {
        const sibling = (child as EditorNode).getPreviousSibling();
        if (sibling === null || needsBlockCursor(sibling)) {
          isBlockCursor = true;
          insertBeforeElement = editor.getElementByKey(
            (child as EditorNode).__key
          );
        }
      }
    }
...
export function $splitNode(
  ...
  ): [ElementNode, ElementNode, EditorNode] => {
...
export function $getChildrenRecursively(node: EditorNode): Array<EditorNode> {
  const nodes = [];
  const stack = [node];
