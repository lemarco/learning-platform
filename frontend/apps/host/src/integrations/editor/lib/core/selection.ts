...
import type { NodeKey } from "./nodes/editor";
import type { ElementNode } from "./nodes/element";
import type { TextFormatType } from "./nodes/text";
// @ts-ignore
...
import { DOM_ELEMENT_TYPE, TEXT_TYPE_TO_FORMAT } from "./constants";
import {
  markCollapsedSelectionFormat,
  markSelectionChangeFromDOMUpdate,
} from "./events";
import { getIsProcesssingMutations } from "./mutations";
...
import { $createTabNode } from "./nodes/tab";
...
function selectPointOnNode(point: PointType, node: EditorNode): void {
  let key = node.__key;
...
export interface BaseSelection {
  clone(): BaseSelection;
  dirty: boolean;
  extract(): Array<EditorNode>;
  getNodes(): Array<EditorNode>;
...
export class NodeSelection implements BaseSelection {
  _nodes: Set<NodeKey>;
  dirty: boolean;
  _cachedNodes: null | Array<EditorNode>;
  ...
  insertNodes(nodes: Array<EditorNode>, selectStart?: boolean): boolean {
...
export class GridSelection implements BaseSelection {
  ...
  focus: PointType;
  dirty: boolean;
  _cachedNodes: Array<EditorNode> | null;
  ...
  getNodes(): Array<EditorNode> {
    ...
    const nodes: Array<EditorNode> = [gridNode!];
    let lastRow = null;
...
export class RangeSelection implements BaseSelection {
   ...
   * converts tabs, newlines, and carriage returns into EditorNodes.
   *
  ...
  insertNodes(nodes: Array<EditorNode>, selectStart?: boolean): boolean {
    ...
    let target: ElementNode | TextNode | DecoratorNode<unknown> | EditorNode =
      anchorNode;
  ...
  insertParagraph(): void {
    ...
    let currentElement;
    let nodesToMove = [];
    let siblingsToMove: Array<EditorNode> = [];
    if (anchor.type === "text") {
      const anchorNode = anchor.getNode();
...
function internalResolveSelectionPoint(
  ...
  let resolvedNode: TextNode | EditorNode | null;
...
export function $getSelectionCore():
  | null
  | RangeSelection
  | NodeSelection
  | GridSelection {
...
export function $insertNodes(
  ...
  let selection = $getSelectionCore() || $getPreviousSelection();
...
export function $getTextContent(): string {
  const selection = $getSelectionCore();
  if (selection === null) {
    return "";
  }
...
export function DEPRECATED_$getNodeTriplet(
  source: PointType | EditorNode | DEPRECATED_GridCellNode
  ...
  if (source instanceof DEPRECATED_GridCellNode) {
    cell = source;
  } else if (source instanceof EditorNode) {
