import type { NodeKey, SerializedEditorNode } from "./editor";
import type {
  GridSelection,
  NodeSelection,
  PointType,
...
export type SerializedElementNode<
  T extends SerializedEditorNode = SerializedEditorNode,
> = Spread<
  {
    children: Array<T>;
...
export class ElementNode extends EditorNode {
  ...
  getChildren<T extends EditorNode>(): Array<T> {
  ...
  getChildrenKeys(): Array<NodeKey> {
    ...
    let child: EditorNode | null = this.getFirstChild();
    while (child !== null) {
  ...
  getFirstDescendant<T extends EditorNode>(): null | T {
  ...
  getLastDescendant<T extends EditorNode>(): null | T {
  ...
  getDescendantByIndex<T extends EditorNode>(index: number): null | T {
  ...
  getFirstChild<T extends EditorNode>(): null | T {
    const self = this.getLatest();
    const firstKey = self.__first;
    return firstKey === null ? null : $getNodeByKey<T>(firstKey);
  }
  getFirstChildOrThrow<T extends EditorNode>(): T {
  ...
  getLastChild<T extends EditorNode>(): null | T {
    const self = this.getLatest();
    const lastKey = self.__last;
    return lastKey === null ? null : $getNodeByKey<T>(lastKey);
  }
  getLastChildOrThrow<T extends EditorNode>(): T {
  ...
  getChildAtIndex<T extends EditorNode>(index: number): null | T {
  ...
  splice(
    start: number,
    deleteCount: number,
    nodesToInsert: Array<EditorNode>
  ): this {
  ...
  canReplaceWith(replacement: EditorNode): boolean {
    return true;
  }
  canInsertAfter(node: EditorNode): boolean {
    return true;
  }
  canBeEmpty(): boolean {
    return true;
