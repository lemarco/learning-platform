...
import { invariant, IS_FIREFOX } from "shared/utils";
...
import { $createLineBreakNode } from "./lineBreak";
import { $createTabNode } from "./tab";
  ...
  return {
    forChild: (EditorNode) => {
      ...
      if (hasBoldFontWeight) {
        EditorNode.toggleFormat("bold");
      }
      if (hasLinethroughTextDecoration) {
        EditorNode.toggleFormat("strikethrough");
      }
      if (hasItalicFontStyle) {
        EditorNode.toggleFormat("italic");
      }
      if (hasUnderlineTextDecoration) {
        EditorNode.toggleFormat("underline");
      }
      if (verticalAlign === "sub") {
        EditorNode.toggleFormat("subscript");
      }
      if (verticalAlign === "super") {
        EditorNode.toggleFormat("superscript");
      }
...
function convertBringAttentionToElement(domNode: Node): DOMConversionOutput {
  ...
  return {
    forChild: (EditorNode) => {
      if ($isTextNode(EditorNode) && !hasNormalFontWeight) {
        EditorNode.toggleFormat("bold");
      }
...
function convertTextFormatElement(domNode: Node): DOMConversionOutput {
  ...
  return {
    forChild: (EditorNode) => {
      if ($isTextNode(EditorNode) && !EditorNode.hasFormat(format)) {
        EditorNode.toggleFormat(format);
      }
