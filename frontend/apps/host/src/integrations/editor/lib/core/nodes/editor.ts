...
import type { EditorConfig, LexicalEditor } from "../editor";
import type {
  GridSelection,
  NodeSelection,
  RangeSelection,
...
export type NodeMap = Map<NodeKey, EditorNode>;
...
export type DOMConversionOutput = {
  after?: (childEditorNodes: Array<EditorNode>) => Array<EditorNode>;
  forChild?: DOMChildConversion;
  node: null | EditorNode | Array<EditorNode>;
};
...
export class EditorNode {
  ...
  //  We set the key in the constructor.
  //@ts-ignore
  __key: string;
  ...
  static getType(): string {
    invariant(
      false,
      "EditorNode: Node %s does not implement .getType().",
      this.name
    );
  }
   ...
   */
  // @ts-ignore
  static clone(_data: unknown): EditorNode {
    invariant(
      false,
      "EditorNode: Node %s does not implement .clone().",
      this.name
    );
  }
  ...
  isAttached(): boolean {
    ...
    while (nodeKey !== null) {
      ...
      const node: EditorNode | null = $getNodeByKey(nodeKey);
   ...
   *
   */
  getPreviousSibling<T extends EditorNode>(): T | null {
   ...
   *
   */
  getPreviousSiblings<T extends EditorNode>(): Array<T> {
   ...
   *
   */
  getNextSibling<T extends EditorNode>(): T | null {
    const self = this.getLatest();
   ...
   *
   */
  getNextSiblings<T extends EditorNode>(): Array<T> {
    const siblings: Array<T> = [];
   ...
   */
  is(object: EditorNode | null | undefined): boolean {
    if (object == null) {
      return false;
    }
   ...
   */
  isBefore(targetNode: EditorNode): boolean {
    if (this === targetNode) {
      return false;
    }
    ...
    let indexA = 0;
    let indexB = 0;
    let node: this | ElementNode | EditorNode = this;
    while (true) {
   ...
   */
  isParentOf(targetNode: EditorNode): boolean {
    const key = this.__key;
    if (key === targetNode.__key) {
      return false;
    }
    let node: ElementNode | EditorNode | null = targetNode;
    while (node !== null) {
      if (node.__key === key) {
        return true;
      }
   ...
   */
  getNodesBetween(targetNode: EditorNode): Array<EditorNode> {
    const isBefore = this.isBefore(targetNode);
    const nodes = [];
    const visited = new Set();
    let node: EditorNode | this = this;
    while (true) {
   ...
   * */
  // @ts-ignore
  static importJSON(_serializedNode: SerializedEditorNode): EditorNode {
    invariant(
      false,
      "EditorNode: Node %s does not implement .importJSON().",
      this.name
    );
  }
  /**
   ...
   */
  static transform(): ((node: EditorNode) => void) | null {
    return null;
  }
  ...
  /**
   * Removes this EditorNode from the EditorState. If the node isn't re-inserted
  ...
  /**
   * Replaces this EditorNode with the provided node, optionally transferring the children
   * of the replaced node to the replacing node.
   *
   * @param replaceWith - The node to replace this one with.
   * @param includeChildren - Whether or not to transfer the children of this node to the replacing node.
   * */
  replace<N extends EditorNode>(replaceWith: N, includeChildren?: boolean): N {
    errorOnReadOnly();
    ...
    if (includeChildren) {
      this.getChildren().forEach((child: EditorNode) => {
  ...
  /**
   * Inserts a node after this EditorNode (as the next sibling).
   *
   * @param nodeToInsert - The node to insert after this one.
   * @param restoreSelection - Whether or not to attempt to resolve the
   * selection to the appropriate place after the operation is complete.
   * */
  insertAfter(nodeToInsert: EditorNode, restoreSelection = true): EditorNode {
    errorOnReadOnly();
  ...
  /**
   * Inserts a node before this EditorNode (as the previous sibling).
   *
   * @param nodeToInsert - The node to insert after this one.
   * @param restoreSelection - Whether or not to attempt to resolve the
   * selection to the appropriate place after the operation is complete.
   * */
  insertBefore(nodeToInsert: EditorNode, restoreSelection = true): EditorNode {
    errorOnReadOnly();
    errorOnInsertTextNodeOnRoot(this, nodeToInsert);
    const writableSelf = this.getWritable();
    const writableNodeToInsert = nodeToInsert.getWritable();
