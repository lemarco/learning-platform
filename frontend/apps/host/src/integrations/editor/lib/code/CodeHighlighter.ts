...
import type { LexicalCommand, LexicalEditor, EditorNode, RangeSelection, NodeSelection, LineBreakNode, GridSelection, NodeKey } from "../";
...
function findNextNonBlankInLine(anchor: EditorNode, offset: number): null | { node: CodeHighlightNode; offset: number } {
  let node: null | EditorNode = anchor;
  let nodeOffset = offset;
  let nodeTextContent = anchor.getTextContent();
...
function getHighlightNodes(tokens: Array<string | Token>, type?: string): EditorNode[] {
...
function getDiffRange(
  prevNodes: Array<EditorNode>,
  nextNodes: Array<EditorNode>,
): {
  from: number;
  nodesForReplacement: Array<EditorNode>;
  to: number;
} {
  let leadingMatch = 0;
...
function isEqual(nodeA: EditorNode, nodeB: EditorNode): boolean {
