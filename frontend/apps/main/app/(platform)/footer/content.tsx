...
const SalesThisWeek: FC<DashboardPageData> = ({ dashboard }) => (
  <div className="rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-6 xl:p-8">
    <div className="mb-4 flex items-center justify-between">
      ...
      <div className="flex flex-1 items-center justify-end text-base font-bold text-green-500 dark:text-green-400">
        {dashboard.salesThisWeek.percentage * 100}%{/* biome-ignore lint/a11y/noSvgWithoutTitle: <explanation> */}
...
const SalesApexChart: FC<DashboardPageData> = ({ dashboard }) => {
...
const NewProductsThisWeek: FC<DashboardPageData> = ({ dashboard }) => (
  <div className="rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-6 xl:p-8">
...
const NewProductsApexChart: FC<DashboardPageData> = ({ dashboard }) => {
  const options: ApexCharts.ApexOptions = {
...
const VisitorsThisWeek: FC<DashboardPageData> = ({ dashboard }) => (
  <div className="rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-6 xl:p-8">
    <div className="flex items-center">
      <div className="shrink-0">
        <span className="text-2xl font-bold leading-none text-gray-900 dark:text-white sm:text-3xl">
...
const VisitorsApexChart: FC<DashboardPageData> = ({ dashboard }) => {
  const { mode } = useThemeMode();
  const isDarkTheme = mode === "dark";
...
const UserSignupsThisWeek: FC<DashboardPageData> = ({ dashboard }) => (
  <div className="rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-6 xl:p-8">
    <div className="flex items-center">
      <div className="shrink-0">
        <span className="text-2xl font-bold leading-none text-gray-900 dark:text-white sm:text-3xl">
...
const UserSignupsApexChart: FC<DashboardPageData> = ({ dashboard }) => {
  const { mode } = useThemeMode();
  const isDarkTheme = mode === "dark";
...
const SessionsByCountry: FC<DashboardPageData> = ({ dashboard }) => (
  <div className="mb-4 rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-6 xl:mb-0 xl:p-8 2xl:col-span-2">
    <div className="mb-4">
      ...
      <span className="text-base font-normal text-gray-600 dark:text-gray-400">View website visitors by hovering over the map</span>
    </div>
    ...
    <ul className="space-y-6">
      <li className="w-full items-center sm:flex">
        <div className="mb-3 flex items-center sm:mb-0">
          ...
          <span className="mx-5 ml-3 w-32 text-base font-medium text-gray-900 dark:text-white sm:flex-none">United States</span>
        </div>
      ...
      <li className="w-full items-center sm:flex">
        <div className="mb-3 flex items-center sm:mb-0">
          ...
          <svg className="h-4 w-4" viewBox="0 0 26 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <mask id="mask0" style={{ maskType: "alpha" }} maskUnits="userSpaceOnUse" x={0} y={0} width={26} height={18}>
              <rect x="0.25" y="0.779053" width="25.2567" height="16.6429" rx="1.75" fill="white" stroke="white" strokeWidth="0.5" />
            </mask>
            <g mask="url(#mask0)">
      ...
      <li className="w-full items-center sm:flex">
        <div className="mb-3 flex items-center sm:mb-0">
          ...
          <svg className="h-4 w-4" viewBox="0 0 26 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <mask id="mask0" style={{ maskType: "alpha" }} maskUnits="userSpaceOnUse" x={0} y={0} width={26} height={18}>
              <rect x="0.25" y="0.779297" width="25.2522" height="16.6429" rx="1.75" fill="white" stroke="white" strokeWidth="0.5" />
            </mask>
            <g mask="url(#mask0)">
      ...
      <li className="w-full items-center sm:flex">
        <div className="mb-3 flex items-center sm:mb-0">
          ...
          <span className="mx-5 ml-3 w-32 flex-none text-base font-medium text-gray-900 dark:text-white">Australia</span>
        </div>
        <div className="h-5 w-full rounded-lg bg-gray-200 dark:bg-gray-700">
          <div
            className="h-5 rounded-md bg-primary-700 p-1 text-center text-xs font-bold leading-none text-primary-100"
...
const SessionsByCountryMap: FC<DashboardPageData> = ({ dashboard }) => {
  const { mode } = useThemeMode();
...
const LatestCustomers: FC<DashboardPageData> = ({ dashboard }) => (
        ...
        {dashboard.latestCustomers.map(({ name, avatar, email, spent }) => (
          <li key={name} className="py-3 sm:py-4">
            <div className="flex items-center space-x-4">
              <div className="shrink-0">
                <Image alt="" height={32} src={avatar} width={32} className="rounded-full" />
              </div>
              ...
              <div className="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">${spent.toFixed(0)}</div>
            </div>
          </li>
        ))}
      </ul>
...
const AcquisitionOverview: FC<DashboardPageData> = ({ dashboard }) => (
  <div className="rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-6 xl:p-8">
                ...
                {dashboard.acquisitionOverview.topChannels.map(({ channel, users, acquisition }) => (
                  <Table.Row key={channel} className="text-gray-500 dark:text-gray-400">
                    ...
                    <Table.Cell className="whitespace-nowrap border-t-0 p-4 align-middle text-xs">
                      <div className="flex items-center">
                        <span className="mr-2 text-xs font-medium">{(acquisition * 100).toFixed(0)}%</span>
                        <div className="relative w-full">
                          <div className="h-2 w-full rounded-sm bg-gray-200 dark:bg-gray-700">
                            <div className="h-2 rounded-sm bg-primary-700" style={{ width: `${acquisition * 100}%` }} />
                          </div>
                        </div>
                      </div>
                    </Table.Cell>
...
const AcquisitionApexChart: FC<DashboardPageData> = ({ dashboard }) => {
  const { mode } = useThemeMode();
  const isDarkTheme = mode === "dark";
...
const Transactions: FC<DashboardPageData> = ({ dashboard }) => {
            ...
            <div className="overflow-hidden shadow sm:rounded-lg">
              <Table striped className="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                ...
                <Table.Body className="bg-white dark:bg-gray-800">
                  {dashboard.transactions.map(({ transaction, date, amount, status }) => (
                    <Table.Row key={`${transaction}-${date}-${amount}`}>
                      <Table.Cell className="whitespace-nowrap p-4 text-sm font-normal text-gray-900 dark:text-white">
                        {transaction}
                      </Table.Cell>
                      <Table.Cell className="whitespace-nowrap p-4 text-sm font-normal text-gray-500 dark:text-gray-400">{date}</Table.Cell>
                      ...
                      <Table.Cell className="flex whitespace-nowrap p-4">
                        <Badge className="rounded-md font-medium" color={getBadgeColor(status)}>
                          {status}
                        </Badge>
                      </Table.Cell>
                    </Table.Row>
