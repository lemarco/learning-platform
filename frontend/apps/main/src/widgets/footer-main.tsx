...
export function FooterMain() {
  return (
    <footer class="bg-white py-12 dark:bg-gray-800 xl:py-24">
...
function Section({ title, items }: Section) {
  return (
    <div>
      <h3 class="mb-6 text-sm font-semibold uppercase text-gray-600 dark:text-white">{title}</h3>
      <ul>
        {items.map((item) => (
...
function SectionItem({ title, href, target = "_blank" }: SectionItem) {
  return (
    <li class="mb-4">
      <a href={href} target={target} rel="noreferrer" class="font-normal text-gray-600 hover:underline dark:text-gray-400">
        {title}
      </a>
    </li>
  );
