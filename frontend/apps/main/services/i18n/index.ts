...
export async function getServerTranslation(language: string, namespace: string | string[], options: { keyPrefix?: string } = {}) {
  const i18nextInstance = await initI18next(language, Array.isArray(namespace) ? namespace[0] : namespace);
...
export const getMetadataGenerator =
  ...
  async ({ params }: Props): Promise<Metadata> => {
    const { t } = await getServerTranslation(params.language, translationSchema);
