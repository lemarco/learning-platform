...
import { LexicalNode, insertRangeAfter } from "./LexicalNode";
...
function $transferStartingElementPointToTextPoint(start: ElementPointType, end: PointType, format: number, style: string): void {
...
export function $INTERNAL_isPointSelection(x: unknown): x is INTERNAL_PointSelection {
  return x instanceof INTERNAL_PointSelection;
}

export function DEPRECATED_$getGridCellNodeRect(GridCellNode: DEPRECATED_GridCellNode): {
  rowIndex: number;
  columnIndex: number;
  rowSpan: number;
  colSpan: number;
...
export class RangeSelection extends INTERNAL_PointSelection {
  ...
  insertText(text: string): void {
    ...
    if (
      ...
      if (
        ...
        ($isElementNode(lastNodeParent) && (!lastNodeParent.canInsertTextBefore() || !lastNodeParent.canInsertTextAfter()))
      ) {
        this.insertText("");
  ...
  insertNodes(nodes: Array<LexicalNode>): void {
    ...
    const notInline = (node: LexicalNode) => ($isElementNode(node) || $isDecoratorNode(node)) && !node.isInline();
...
function getCharacterOffsets(selection: INTERNAL_PointSelection): [number, number] {
...
function shouldResolveAncestor(resolvedElement: ElementNode, resolvedOffset: number, lastPoint: null | PointType): boolean {
  const parent = resolvedElement.getParent();
  return lastPoint === null || parent === null || !parent.canBeEmpty() || parent !== lastPoint.getNode();
}

function internalResolveSelectionPoint(dom: Node, offset: number, lastPoint: null | PointType, editor: LexicalEditor): null | PointType {
...
function normalizeSelectionPointsForBoundaries(anchor: PointType, focus: PointType, lastSelection: null | BaseSelection): void {
