import type { CodeNode } from "../../lexical-code/LexicalCode";
import type { ElementTransformer, TextFormatTransformer, TextMatchTransformer, Transformer } from "../../lexical-markdown/LexicalMarkdown";
import type { LexicalNode, TextNode } from "../../lexical/Lexical";

import { $createCodeNode } from "../../lexical-code/LexicalCode";
import { $isListItemNode, $isListNode, ListItemNode } from "../../lexical-list/LexicalList";
import { $isQuoteNode } from "../../lexical-rich-text/LexicalRichText";
...
import { IS_APPLE_WEBKIT, IS_IOS, IS_SAFARI } from "../../shared/src/environment";
...
export function createMarkdownImport(transformers: Array<Transformer>): (markdownString: string, node?: ElementNode) => void {
...
function isEmptyParagraph(node: LexicalNode): boolean {
  ...
  return (
    firstChild == null ||
    (node.getChildrenSize() === 1 && $isTextNode(firstChild) && MARKDOWN_EMPTY_LINE_REG_EXP.test(firstChild.getTextContent()))
  );
}
...
function importCodeBlock(lines: Array<string>, startLineIndex: number, rootNode: ElementNode): [CodeNode | null, number] {
...
function findOutermostMatch(textContent: string, textTransformersIndex: TextFormatTransformersIndex): RegExpMatchArray | null {
...
function createTextFormatTransformersIndex(textTransformers: Array<TextFormatTransformer>): TextFormatTransformersIndex {
