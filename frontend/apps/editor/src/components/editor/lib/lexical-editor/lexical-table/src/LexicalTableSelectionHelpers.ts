...
export function applyTableHandlers(
  ...
  tableSelection.listenersToRemove.add(
    editor.registerCommand<KeyboardEvent>(
      KEY_ARROW_DOWN_COMMAND,
      (event) => $handleArrowKey(editor, event, "down", tableNode, tableSelection),
      COMMAND_PRIORITY_HIGH,
    ),
  );
  ...
  tableSelection.listenersToRemove.add(
    editor.registerCommand<KeyboardEvent>(
      KEY_ARROW_UP_COMMAND,
      (event) => $handleArrowKey(editor, event, "up", tableNode, tableSelection),
      COMMAND_PRIORITY_HIGH,
    ),
  );
  ...
  tableSelection.listenersToRemove.add(
    editor.registerCommand<KeyboardEvent>(
      KEY_ARROW_LEFT_COMMAND,
      (event) => $handleArrowKey(editor, event, "backward", tableNode, tableSelection),
      COMMAND_PRIORITY_HIGH,
    ),
  );
  ...
  tableSelection.listenersToRemove.add(
    editor.registerCommand<KeyboardEvent>(
      KEY_ARROW_RIGHT_COMMAND,
      (event) => $handleArrowKey(editor, event, "forward", tableNode, tableSelection),
      COMMAND_PRIORITY_HIGH,
    ),
  );
  ...
  const deleteTextHandler = (command: LexicalCommand<boolean>) => () => {
    ...
    if ($isGridSelection(selection)) {
      ...
      const topLevelCellElementNode =
        nearestElementNode && $findMatchingParent(nearestElementNode, (n) => $isElementNode(n) && $isTableCellNode(n.getParent()));
  ...
  tableSelection.listenersToRemove.add(
    editor.registerCommand(
      ...
      (selectionPayload) => {
        ...
        if (nodes.length !== 1 || !DEPRECATED_$isGridNode(nodes[0]) || !isSelectionInsideOfGrid) {
          return false;
        }
        ...
        const gridCellNode = $findMatchingParent(anchor.getNode(), (n) => DEPRECATED_$isGridCellNode(n));
...
export function getTableSelectionFromTableElement(tableElement: HTMLTableElementWithWithTableSelectionState): TableSelection | null {
...
export function $updateDOMForSelection(editor: LexicalEditor, grid: Grid, selection: GridSelection | RangeSelection | null) {
...
function $isSelectionInTable(selection: null | BaseSelection, tableNode: TableNode): boolean {
