...
export { markSelection } from "./markSelection";
export { mergeRegister } from "./mergeRegister";
export { positionNodeOnRange } from "./positionNodeOnRange";
 ...
 */
export function addClassNamesToElement(element: HTMLElement, ...classNames: Array<typeof undefined | boolean | null | string>): void {
 ...
 */
export function removeClassNamesFromElement(element: HTMLElement, ...classNames: Array<typeof undefined | boolean | null | string>): void {
  // biome-ignore lint/complexity/noForEach: <explanation>
  classNames.forEach((className) => {
    if (typeof className === "string") {
 ...
 * }));
 */
export function mediaFileReader(files: Array<File>, acceptableMimeTypes: Array<string>): Promise<Array<{ file: File; result: string }>> {
 ...
 */
export function objectKlassEquals<T>(object: unknown, objectClass: ObjectKlass<T>): boolean {
...
export function $filter<T>(nodes: Array<LexicalNode>, filterFn: (node: LexicalNode) => null | T): Array<T> {
  const result: T[] = [];
