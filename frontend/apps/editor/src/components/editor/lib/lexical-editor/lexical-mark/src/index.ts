...
import type { LexicalNode, RangeSelection, TextNode } from "../../lexical/Lexical";
import type { SerializedMarkNode } from "./MarkNode";

import { $isElementNode, $isTextNode } from "../../lexical/Lexical";
...
export function $wrapSelectionInMarkNode(
  ...
  for (let i = 0; i < nodesLength; i++) {
    ...
    if ($isTextNode(node)) {
      ...
      targetNode =
        splitNodes.length > 1 && (splitNodes.length === 3 || (isFirstNode && !isLastNode) || endTextOffset === textContentSize)
          ? splitNodes[1]
          : splitNodes[0];
