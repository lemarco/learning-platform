...
import type { DecoratorNode, ElementNode, LexicalNode, NodeMap, RangeSelection, TextNode } from "../..";

import { $getNodeByKey, $isDecoratorNode, $isElementNode, $isLineBreakNode, $isRootNode, $isTextNode, createEditor, NodeKey } from "../..";
...
const baseExcludedProperties = new Set<string>(["__key", "__parent", "__next", "__prev"]);
const elementExcludedProperties = new Set<string>(["__first", "__last", "__size"]);
const rootExcludedProperties = new Set<string>(["__cachedText"]);
const textExcludedProperties = new Set<string>(["__text"]);

function isExcludedProperty(name: string, node: LexicalNode, binding: Binding): boolean {
