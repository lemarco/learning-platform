import { $createLinkNode, $isLinkNode, LinkNode, SerializedLinkNode, toggleLink } from "@lexical/link";
import { $getRoot, $selectAll, ParagraphNode, SerializedParagraphNode, TextNode } from "lexical/src";
  ...
  initializeUnitTest((testEnv) => {
    ...
    test("LinkNode.createDOM()", async () => {
      ...
      await editor.update(() => {
        ...
        expect(linkNode.createDOM(editorConfig).outerHTML).toBe('<a href="https://example.com/foo" class="my-link-class"></a>');
        expect(
          linkNode.createDOM({
            namespace: "",
            theme: {},
