import { $generateHtmlFromNodes, $generateNodesFromDOM } from "../../lexical-html/LexicalHtml";
import { $addNodeStyle, $cloneWithProperties, $sliceSelectedTextNodeContent } from "../../lexical-selection/LexicalSelection";
import { objectKlassEquals } from "../../lexical-utils/LexicalUtils";
import {
  $createTabNode,
  $getRoot,
  $getSelection,
 ...
 */
export function $insertDataTransferForRichText(dataTransfer: DataTransfer, selection: BaseSelection, editor: LexicalEditor): void {
 ...
 */
export function $insertGeneratedNodes(editor: LexicalEditor, nodes: Array<LexicalNode>, selection: BaseSelection): void {
  if (
...
function exportNodeToJSON<T extends LexicalNode>(node: T): BaseSerializedNode {
  ...
  if ($isElementNode(node)) {
    ...
    if (!Array.isArray(serializedChildren)) {
      invariant(false, "LexicalNode: Node %s is an element but .exportJSON() does not have a children array.", nodeClass.name);
    }
  }
