...
import { mergeRegister } from "../../lexical-utils/LexicalUtils";
import {
  $createLineBreakNode,
  $createTabNode,
  $createTextNode,
...
function findNextNonBlankInLine(anchor: LexicalNode, offset: number): null | { node: CodeHighlightNode; offset: number } {
...
function getHighlightNodes(tokens: Array<string | Token>, type?: string): LexicalNode[] {
...
export function registerCodeHighlighting(editor: LexicalEditor, tokenizer?: Tokenizer): () => void {
  ...
  return mergeRegister(
    ...
    editor.registerCommand(MOVE_TO_START, (payload): boolean => handleMoveTo(MOVE_TO_START, payload), COMMAND_PRIORITY_LOW),
  );
}
