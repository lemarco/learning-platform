...
import { KeyboardEventMock, initializeUnitTest, invariant, shiftTabKeyboardEvent, tabKeyboardEvent } from "lexical/src/__tests__/utils";
...
describe("LexicalCodeNode tests", () => {
  initializeUnitTest((testEnv) => {
    ...
    test("CodeNode.createDOM()", async () => {
      ...
      await editor.update(() => {
        const codeNode = $createCodeNode();
        expect(codeNode.createDOM(editorConfig).outerHTML).toBe('<code class="my-code-class" spellcheck="false"></code>');
        expect(
          codeNode.createDOM({
            namespace: "",
            theme: {},
...
2"><span data-lexical-text="true">hello</span><span data-lexical-text="true">\t</span><span data-lexical-text="true">world</span><br><span data-lexical-text="true">hello</span><span data-lexical-text="true">\t</span><span data-lexical-text="true">world</span></code>`,
    ...
    test("can indent at the start of the second line", async () => {
      ...
      await editor.dispatchCommand(KEY_TAB_COMMAND, tabKeyboardEvent());
      expect(testEnv.innerHTML).toBe(`<code spellcheck="false" data-highlight-language="javascript" dir="ltr" data-gutter="1
      ...
      for (const moveTo of ["start", "end"]) {
        for (const tabOrSpaces of ["tab", "spaces"]) {
          ...
          testMoveTo(
            ...
            () => {
              const nodes = $dfs();
              const codeHighlight = nodes.filter((dfsNode) => $isCodeHighlightNode(dfsNode.node))[tabOrSpaces === "tab" ? 0 : 1].node;
          ...
          testMoveTo(
            ...
            () => {
              const nodes = $dfs();
              const codeHighlight = nodes.filter((dfsNode) => $isCodeHighlightNode(dfsNode.node))[tabOrSpaces === "tab" ? 1 : 2].node;
