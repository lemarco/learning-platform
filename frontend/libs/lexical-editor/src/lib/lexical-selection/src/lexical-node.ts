...
export function $sliceSelectedTextNodeContent(selection: BaseSelection, textNode: TextNode): LexicalNode {
  if (textNode.isSelected(selection) && !textNode.isSegmented() && !textNode.isToken() && $INTERNAL_isPointSelection(selection)) {
 ...
 */
export function $patchStyleText(selection: INTERNAL_PointSelection, patch: Record<string, string | null>): void {
  ...
  if (!$isRangeSelection(selection)) {
    ...
    for (let i = 0; i < selectedNodesLength; i++) {
      ...
      if (DEPRECATED_$isGridCellNode(node)) {
        ...
        $patchStyleText($normalizeSelection__EXPERIMENTAL(cellSelection), patch);
      }
    }
