 ...
 */
export function $setBlocksType(selection: INTERNAL_PointSelection, createElement: () => ElementNode): void {
...
export function $wrapNodesImpl(
  ...
  for (let i = 0; i < nodesLength; i++) {
    ...
    if (parent !== null && $isLeafNode(node) && !movedNodes.has(node.getKey())) {
      ...
      invariant($isElementNode(node), "Expected node in emptyElements to be an ElementNode");
      const targetElement = createElement();
      targetElement.setFormat(node.getFormatType());
      targetElement.setIndent(node.getIndent());
      elements.push(targetElement);
 ...
 */
export function $getSelectionStyleValueForProperty(selection: RangeSelection, styleProperty: string, defaultValue = ""): string {
