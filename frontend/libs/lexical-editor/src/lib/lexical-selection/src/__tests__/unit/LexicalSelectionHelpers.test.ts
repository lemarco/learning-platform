...
describe("LexicalSelectionHelpers tests", () => {
  ...
  describe("can insert block element nodes correctly", () => {
    ...
    test("Can insert link into empty paragraph", async () => {
      ...
      expect(element.innerHTML).toBe('<p><a href="https://lexical.dev" dir="ltr"><span data-lexical-text="true">Lexical</span></a></p>');
    });
    ...
    test("Can insert an ElementNode after ShadowRoot", async () => {
      ...
      expect(["<div><br></div><div><br></div>", "<div><br></div><p><br></p>"]).toContain(element.innerHTML);
    });
  });
});
...
describe("insertNodes", () => {
  ...
  it("can insert element next to top level decorator node", async () => {
    ...
    await editor.update(() => {
      $getRoot().append($createParagraphNode(), $createTestDecoratorNode(), $createParagraphNode().append($createTextNode("Text after")));
    });
  ...
  it("can insert when previous selection was null", async () => {
    ...
    await editor.update(() => {
      ...
      selection.insertNodes([$createParagraphNode().append($createTextNode("Text"))]);
    ...
    await editor.update(() => {
      ...
      selection.insertNodes([$createParagraphNode().append($createTextNode("Before "))]);
  ...
  it("can insert when before empty text node", async () => {
    ...
    await editor.update(() => {
      ...
      $getRoot().append($createParagraphNode().append(emptyTextNode, $createTextNode("text")));
  ...
  it("last node is LineBreakNode", async () => {
    ...
    await editor.update(() => {
      ...
      const newHeading = $createHeadingNode("h1").append($createTextNode("heading"));
      selection.insertNodes([newHeading, $createLineBreakNode()]);
    });
    editor.getEditorState().read(() => {
      expect(element.innerHTML).toBe('<h1 dir="ltr"><span data-lexical-text="true">heading</span></h1><p><br></p>');
