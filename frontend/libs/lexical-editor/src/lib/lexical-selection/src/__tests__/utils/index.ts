...
if (!Selection.prototype.modify) {
  ...
  const pushSegment = (segments: Array<Segment>, index: number, str: string, isWordLike: boolean): void => {
    segments.push({
  ...
  const getWordsFromString = (string: string): Array<Segment> => {
    const segments = [];
    let wordString = "";
    let nonWordString = "";
    let i;
  ...
  Selection.prototype.modify = function (alter, direction, granularity) {
    ...
    if (granularity === "character") {
      ...
      const targetTextContent =
        direction === "backward" ? anchorNode.textContent.slice(0, this.anchorOffset) : anchorNode.textContent.slice(this.anchorOffset);
      ...
      await update(() => {
        ...
        switch (input.type) {
          ...
          case "move_native_selection": {
            setNativeSelectionWithPaths(rootElement, input.anchorPath, input.anchorOffset, input.focusPath, input.focusOffset);
            break;
          }
