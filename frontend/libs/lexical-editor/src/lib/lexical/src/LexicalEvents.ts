...
import { COMPOSITION_START_CHAR, DOM_ELEMENT_TYPE, DOM_TEXT_TYPE, DOUBLE_LINE_BREAK, IS_ALL_FORMATTING } from "./LexicalConstants";
import { RangeSelection, internalCreateRangeSelection } from "./LexicalSelection";
    ...
    if ($isRangeSelection(selection)) {
      ...
      if (selection.isCollapsed()) {
        ...
        if (currentTimeStamp < timeStamp + 200 && anchor.offset === lastOffset && anchor.key === lastKey) {
          ...
          if (anchor.type === "text") {
            invariant($isTextNode(anchorNode), "Point.getNode() must return TextNode when type is text");
...
function $canRemoveText(anchorNode: TextNode | ElementNode, focusNode: TextNode | ElementNode): boolean {
  return (
    anchorNode !== focusNode || $isElementNode(anchorNode) || $isElementNode(focusNode) || !anchorNode.isToken() || !focusNode.isToken()
  );
}
  ...
  updateEditor(editor, () => {
    ...
    if (
      ...
      if (
        ...
        anchorNode.getTextContent().slice(0, offset) + data + anchorNode.getTextContent().slice(offset + selection.focus.offset) !==
...
export function markCollapsedSelectionFormat(format: number, style: string, offset: number, key: NodeKey, timeStamp: number): void {
