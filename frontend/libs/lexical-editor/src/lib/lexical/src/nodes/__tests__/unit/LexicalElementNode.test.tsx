...
describe("LexicalElementNode tests", () => {
  ...
  describe("getTextContentSize()", () => {
    test("basic", async () => {
      await update(() => {
        expect($getRoot().getFirstChild()!.getTextContentSize()).toBe($getRoot().getFirstChild()!.getTextContent().length);
      });
    });
  ...
  describe("splice", () => {
    ...
    BASE_INSERTIONS.forEach((testCase) => {
      it(`Plain text: ${testCase.name}`, async () => {
        await update(() => {
          block.splice(testCase.start, testCase.deleteCount, testCase.deleteOnly ? [] : [$createTextNode("Qux"), $createTextNode("Quuz")]);
    ...
    it("Running transforms for inserted nodes, their previous siblings and new siblings", async () => {
      ...
      await update(() => {
        ...
        // transformer calls
        expectedTransforms.push(text1.__key, text2.__key, text3.__key, block.getChildAtIndex(0)!.__key, block.getChildAtIndex(1)!.__key);
      });
