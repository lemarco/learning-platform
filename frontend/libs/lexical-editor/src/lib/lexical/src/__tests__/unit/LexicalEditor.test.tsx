...
import { $createTableCellNode, $createTableNode, $createTableRowNode, TableCellNode, TableRowNode } from "@lexical/table";
import {
  $createLineBreakNode,
  $createNodeSelection,
  $createParagraphNode,
...
import { ReactNode, createRef, useCallback, useEffect, useMemo, useState } from "react";
...
describe("LexicalEditor tests", () => {
  ...
  function useLexicalEditor(rootElementRef: React.RefObject<HTMLDivElement>, onError?: (error: Error) => void) {
  ...
  it("Synchronously runs three transforms, two of them depend on the other (2)", async () => {
    ...
    const italicsListener = editor.registerNodeTransform(ParagraphNode, (paragraph) => {
      ...
      if ($isTextNode(child) && child.hasFormat("bold") && !child.hasFormat("italic")) {
  ...
  it("removes the command from the command map when no listener are attached", () => {
    ...
    expect(editor._commands).toEqual(
      new Map([[command, [new Set([commandListener, commandListenerTwo]), new Set(), new Set(), new Set(), new Set()]]]),
    );

    removeCommandListener();

    expect(editor._commands).toEqual(new Map([[command, [new Set([commandListenerTwo]), new Set(), new Set(), new Set(), new Set()]]]));
  ...
  it('mutation "update" listener', async () => {
    ...
    await editor.update(() => {
      const paragraphNode1 = $getNodeByKey(paragraphNodeKeys[0]) as ParagraphNode;
      const textNode1 = $createTextNode("foo");
  ...
  it("does not include linebreak into inline elements", async () => {
    ...
    expect(container.firstElementChild?.innerHTML).toBe('<p dir="ltr"><span data-lexical-text="true">Hello</span><a></a></p>');
  });
  ...
  it("recovers from reconciler failure and trigger proper prev editor state", async () => {
    ...
    await update(() => {
      $getRoot().append($createParagraphNode().append($createTextNode("world")));
    });
