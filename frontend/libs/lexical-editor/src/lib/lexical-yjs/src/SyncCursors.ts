...
import type { BaseSelection, NodeKey, NodeMap, Point } from "@frontend/lexical-editor";
import type { AbsolutePosition, RelativePosition } from "yjs";

import { createDOMRange, createRectsFromDOMRange } from "../../lexical-selection/src";
import { $getNodeByKey, $getSelection, $isElementNode, $isLineBreakNode, $isRangeSelection, $isTextNode } from "@frontend/lexical-editor";
...
function shouldUpdatePosition(currentPos: RelativePosition | null | undefined, pos: RelativePosition | null | undefined): boolean {
  if (currentPos == null) {
    if (pos != null) {
      return true;
    }
...
function updateCursor(binding: Binding, cursor: Cursor, nextSelection: null | CursorSelection, nodeMap: NodeMap): void {
  ...
  for (let i = 0; i < selectionRectsLength; i++) {
    ...
    (selection.firstChild as HTMLSpanElement).style.cssText = `${style}left:0;top:0;background-color:${color};opacity:0.3;`;
