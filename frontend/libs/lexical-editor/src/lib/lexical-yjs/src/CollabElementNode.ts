...
import type { ElementNode, NodeKey, NodeMap } from "@frontend/lexical-editor";
import type { AbstractType, XmlElement, XmlText } from "yjs";

import { $getNodeByKey, $isDecoratorNode, $isElementNode, $isTextNode } from "@frontend/lexical-editor";
    ...
    for (let i = 0; i < deltas.length; i++) {
      ...
      if (delta.retain != null) {
        ...
        if (typeof insertDelta === "string") {
          ...
          const collabNode = getOrInitCollabNodeFromSharedType(binding, sharedType as XmlText | YMap<unknown> | XmlElement, this);
...
export function $createCollabElementNode(xmlText: XmlText, parent: null | CollabElementNode, type: string): CollabElementNode {
