import type { DEPRECATED_GridRowNode, ElementNode } from "../../lexical/Lexical";
...
export function $insertTableRow__EXPERIMENTAL(insertAfter = true): void {
  ...
  invariant($INTERNAL_isPointSelection(selection), "Expected a INTERNAL_PointSelection");
  ...
  if (insertAfter) {
    ...
    for (let i = 0; i < columnCount; i++) {
      ...
      if (startRow + cell.__rowSpan - 1 <= focusEndRow) {
        newRow.append($createTableCellNode(TableCellHeaderStates.NO_STATUS).append($createParagraphNode()));
      } else {
