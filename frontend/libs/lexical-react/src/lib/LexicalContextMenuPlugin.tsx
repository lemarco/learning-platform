...
import { COMMAND_PRIORITY_LOW, CommandListenerPriority, LexicalNode } from "@frontend/lexical-editor";
...
export function LexicalContextMenuPlugin<TOption extends MenuOption>({
  ...
  const handleClick = useCallback(
    (event: MouseEvent) => {
      if (resolution !== null && menuRef.current != null && event.target != null && !menuRef.current.contains(event.target as Node)) {
        closeNodeMenu();
      }
    },
