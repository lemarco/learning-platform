...
import { $generateHtmlFromNodes } from "@frontend/lexical-editor";
import { $isLinkNode, LinkNode } from "@frontend/lexical-editor";
import { $isMarkNode } from "@frontend/lexical-editor";
import { $isGridSelection, GridSelection } from "@frontend/lexical-editor";
...
function generateContent(
  ...
  const selectionString = editorState.read(() => {
    ...
    visitTree($getRoot(), (node: LexicalNode, indent: Array<string>) => {
      ...
      res += `${isSelected ? SYMBOLS.selectedLine : " "} ${indent.join(" ")} ${nodeKeyDisplay} ${typeDisplay} ${idsDisplay} ${printNode(
        node,
      )}\n`;
...
function visitTree(currentNode: ElementNode, visitor: (node: LexicalNode, indentArr: Array<string>) => void, indent: Array<string> = []) {
...
const MODE_PREDICATES = [(node: TextNode) => node.isToken() && "Token", (node: TextNode) => node.isSegmented() && "Segmented"];
...
function printSelectedCharsLine({
  ...
  return (
    [SYMBOLS.selectedLine, indentionChars.join(" "), [...nodePrintSpaces, ...unselectedChars, ...selectedChars].join("")].join(" ") + "\n"
  );
}
...
function $getSelectionStartEnd(node: LexicalNode, selection: INTERNAL_PointSelection): [number, number] {
  const anchor = selection.anchor;
  const focus = selection.focus;
