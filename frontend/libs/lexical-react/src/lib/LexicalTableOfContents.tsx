import type { LexicalEditor, NodeKey, NodeMutation } from "@frontend/lexical-editor";

import { useLexicalComposerContext } from "./LexicalComposerContext";
import { $isHeadingNode, HeadingNode, HeadingTagType } from "@frontend/lexical-editor";
import { $getNodeByKey, $getRoot, TextNode } from "@frontend/lexical-editor";
...
function $deleteHeadingFromTableOfContents(key: NodeKey, currentTableOfContents: Array<TableOfContentsEntry>): Array<TableOfContentsEntry> {
...
export function LexicalTableOfContentsPlugin({ children }: Props): JSX.Element {
  ...
  useEffect(() => {
    ...
    const removeHeaderMutationListener = editor.registerMutationListener(HeadingNode, (mutatedNodes: Map<string, NodeMutation>) => {
      editor.getEditorState().read(() => {
    ...
    const removeTextNodeMutationListener = editor.registerMutationListener(TextNode, (mutatedNodes: Map<string, NodeMutation>) => {
      editor.getEditorState().read(() => {
        for (const [nodeKey, mutation] of mutatedNodes) {
          if (mutation === "updated") {
