...
export function useHover(context: any, props: any = {}) {
  const {
    open,
    onOpenChange,
    dataRef,
  ...
  const closeWithDelay = (event: Event, runElseBranch = true, reason = "hover") => {
  ...
  React.useEffect(() => {
    ...
    function onMouseLeave(event: MouseEvent) {
      ...
      // consistently.
      const shouldClose = pointerTypeRef.current === "touch" ? !contains(floating, event.relatedTarget as Element | null) : true;
      if (shouldClose) {
