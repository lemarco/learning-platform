import type { Boundary, ClientRectObject, Rect, RootBoundary, Strategy } from "@floating-ui/core";
...
function getClippingElementAncestors(element: Element, cache: PlatformWithCache["_c"]): Array<Element> {
  const cachedResult = cache.get(element);
  if (cachedResult) {
    return cachedResult;
  }

  let result = getOverflowAncestors(element, [], false).filter((el) => isElement(el) && getNodeName(el) !== "body") as Array<Element>;
