import { contains, getDocument, getTarget, isEventTargetWithin, isReactEvent, isRootElement } from "@floating-ui/react/utils";
...
export const normalizeProp = (normalizable?: boolean | { escapeKey?: boolean; outsidePress?: boolean }) => {
  return {
...
export function useDismiss<RT extends ReferenceType = ReferenceType>(
  ...
  const closeOnEscapeKeyDown = useEffectEvent((event: React.KeyboardEvent<Element> | KeyboardEvent) => {
  ...
  const closeOnPressOutside = useEffectEvent((event: MouseEvent) => {
    ...
    const targetIsInsideChildren =
      tree && getChildren(tree.nodesRef.current, nodeId).some((node) => isEventTargetWithin(event, node.context?.elements.floating));
