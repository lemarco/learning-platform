 ...
 */
export function useFloating<RT extends ReferenceType = ReferenceType>(options: Partial<UseFloatingOptions> = {}): UseFloatingReturn<RT> {
  ...
  const [_domReference, setDomReference] = React.useState<NarrowedElement<RT> | null>(null);

  const domReference = (options.elements?.reference || _domReference) as NarrowedElement<RT>;

  const position = usePosition<RT>(options);
  const tree = useFloatingTree<RT>();
  const nested = useFloatingParentNodeId() != null;

  const onOpenChange = useEffectEvent((open: boolean, event?: Event, reason?: OpenChangeReason) => {
    if (open) {
