 ...
 */
export function safePolygon<RT extends ReferenceType = ReferenceType>(options: Partial<SafePolygonOptions> = {}) {
  ...
  const fn: HandleCloseFn<RT> = ({ x, y, placement, elements, onClose, nodeId, tree }) => {
    return function onMouseMove(event: MouseEvent) {
      ...
      if (tree && getChildren(tree.nodesRef.current, nodeId).some(({ context }) => context?.open)) {
        return;
      }
